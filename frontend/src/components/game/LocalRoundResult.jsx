...
export default function LocalRoundResult({ result, onNext }) {
  if (!result) return null;
  const { correct, song, isLastRound } = result;
  ...
  return (
    <AnimatePresence>
      ...
      >
        <motion.div
          className={`rounded-2xl p-8 text-center max-w-sm mx-4 shadow-2xl border-2
            ${correct ? 'bg-green-900/80 border-green-400' : 'bg-red-900/80 border-red-400'}`}
        ...
        >
          ...
          <Button fullWidth onClick={onNext}>
            {isLastRound ? '🏆 See Results' : '→ Next Turn'}
          </Button>
        </motion.div>
      </motion.div>
    </AnimatePresence>
